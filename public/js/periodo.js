document.addEventListener("DOMContentLoaded", function () {    const body = document.body;
    ...
    const infoPeriodo = document.getElementById("info-periodo-linea");

    if (infoPeriodo) {
        infoPeriodo.style.display = "flex";
        infoPeriodo.style.justifyContent = "center";
        infoPeriodo.style.alignItems = "center";
        infoPeriodo.style.gap = "60px";
        infoPeriodo.style.padding = "0 80px";
        infoPeriodo.style.fontSize = "15px";
        infoPeriodo.style.fontWeight = "500";
        infoPeriodo.style.color = "#1a2744";
        infoPeriodo.style.marginBottom = "20px";

        infoPeriodo.querySelectorAll("span").forEach(span => {
            span.style.whiteSpace = "nowrap";
        });
    }
    const formPeriodo = document.querySelector(".form-periodo");
    document.querySelectorAll(".form-periodo .form-control").forEach(input => {
    ...
    document.querySelectorAll(".form-periodo label").forEach(label => {
...
function confirmarPeriodo() {
    return confirm(
        "¿Estás seguro de registrar este nuevo periodo?\n\n" +
        "El periodo actual quedará cerrado."
    );
}